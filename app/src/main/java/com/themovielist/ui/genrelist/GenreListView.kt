...
class GenreListView(context: Context?, attrs: AttributeSet?) : FrameLayout(context, attrs) {
    private val mGenreListAdapter by lazy { GenreListAdapter(string.the_list_is_empty) }

    var onSelectGenreListener: ((position: Int, genreListItemModel: GenreListItemModel) -> Unit)? = null

    init {
        LayoutInflater.from(context).inflate(layout.genre_list_view, this)
    ...
    private fun configureRecyclerView() {
        ...
        val dividerAmountOfSpace = context.resources.getDimension(dimen.genre_horizontal_space)
